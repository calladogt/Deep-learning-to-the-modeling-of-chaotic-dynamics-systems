...
def train(seq, filename, max_epochs, device):
    ...
    str_settings = "Model : " + str(seq) + ", trained with MGD\n\n"
...
def train_loop(seq, filename, max_epochs, device, num_train=3, max_failures=10):
    ...
    while i < num_train or fails == max_failures:
        global losses
        losses = []
        trained = train(seq, "training_intnn_mgd", max_epochs, device)
        if trained:
            shutil.move("training_intnn_mgd", filename+"/training"+str(i))
            i += 1
        else:
            fails += 1
