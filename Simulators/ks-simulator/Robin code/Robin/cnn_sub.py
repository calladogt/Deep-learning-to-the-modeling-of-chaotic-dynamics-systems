...
def train_loop(filename, max_epochs, subsampling, delta_t, device, num_train=2):
    ...
    for i in range(num_train):
        global losses
        losses = []
        trained = train("training_cnn_sub", max_epochs, subsampling, delta_t, device)
        if trained:
            shutil.move("training_cnn_sub", filename+"/training"+str(i))
