...
from lyapunov_exponents import stable_le, lyapunov_exponent_fulltraj_resNN
...
def train(filename, max_epochs, subsampling, delta_t, device):
    ...
    optimizer.param_groups[0]['tolerance_grad'] = 1e-12
    ...
    str_settings += str(nb_steps)+" steps, delta t = "+str(delta_t)+" * "+str(subsampling) + ", init pos = " + str(pos)

    us.save_settings(filename, str_settings)

    def res_net(inp, out):
        return inp + delta_t * subsampling * out

    def closure():
        optimizer.zero_grad()
        out = res_net(input, seq(input))

        loss = criterion(out, target)
        print('loss:', loss.item())
        print('loss / delta t:', (loss.item() / delta_t))
        us.save_trainlog(logfile, 'loss :' + str(loss.item()))
        us.save_trainlog(logfile, 'loss / delta t:' + str(loss.item() / delta_t))
        loss.backward()
    ...
    for i in range(1, max_epochs + 1):
        ...
        if (loss_before - losses[-1]) < tol_change:
            optimizer.param_groups[0]['lr'] /= 2
            print("lr divided by 2, new value =", optimizer.param_groups[0]['lr'])

        if optimizer.param_groups[0]['lr'] < min_lr or i == max_epochs:
        # if abs(loss_before - losses[-1]) < tol_change or i == max_epochs:
            ...
            up.plot_singval_resnn(seq, data, Lorenz, delta_t * subsampling, nb_steps, 0, 5000, i, filename+"/jacobians")
            up.plot_losses(losses, filename, "all_losses")
            up.plot_traj(res_net(input, seq(input)), "Final_pred", filename)
            ...
            with torch.no_grad():
                ...
                pred = res_net(input1, seq(input1))
            ...
            lyap_nn = lyapunov_exponent_fulltraj_resNN(seq.jacobian, preds, delta_t * subsampling)
...
def train_loop(filename, max_epochs, subsampling, delta_t, device, num_train=3):
    if os.path.exists(filename):
        shutil.rmtree(filename)
    for i in range(num_train):
        global losses
        losses = []
        trained = train("training_resnn_sub", max_epochs, subsampling, delta_t, device)
        if trained:
            shutil.move("training_resnn_sub", filename+"/training"+str(i))

def full_train():
    delta_t = [0.1, 0.05, 0.025, 1e-2, 5e-3, 2.5e-3, 1e-3, 5e-4, 2.5e-4, 1e-4]
    subsampling = [1]

    for dt in delta_t:
        for ssp in subsampling:
            name = "resnn_sub"+str(ssp)+"_dt"+str(dt)
            train_loop(name, 100, ssp, dt, device)
